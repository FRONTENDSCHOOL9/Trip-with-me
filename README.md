...
```
멋쟁이사차럼 프런트엔드 9기 프로젝트( Trip-with-me 1조 )를 진행하면서 제가 맡은 각 콘텐츠 부분이 밸리데이션을 거쳐 동작 되는 기능들이 많았습니다.
저희가 원하는 결과를 미리 Trip-with-me 서비스에 설정된 기준에 사용자가 맞춰 적합한 결과를 얻어냈는지까지 직접 그 사용자가 되어보고 불편한 것과 부족한 안내 등 어떻게 더욱 심플하게 사용자에게 사용하기 쉽게 다가갈 수 있을까 최소한의 내용으로 사용자에게 원하는 결과물을 어떻게 얻어낼까? 많은 고민을 했던 시간이였습니다.
그리고 Product 추가 하는 기능구현을 하면서 많은 에러와 오류가 있었습니다..
이 부분에서 서버와 데이터에 한걸음 가까워진 거 같습니다.
저희 프로젝트 주제 처럼 같이 동행해 준 1조 팀원들에게 감사드립니다. 감사합니다.
```
